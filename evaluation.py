...
def result(pred, true):
    ...
    return metrics.roc_auc_score(true, aa), area
...
def node_classification_evaluation(model, inductive_dataset, graph, x, num_classes, lr_f, weight_decay_f, max_epoch_f, device, sche, linear_prob=True, mute=False):
    ...
    final_aupr, estp_aupr = linear_probing_for_transductive_node_classiifcation(encoder, inductive_dataset, graph, x, optimizer_f, max_epoch_f, device, sche, mute)
    return final_aupr, estp_aupr
...
def linear_probing_for_transductive_node_classiifcation(model, inductive_dataset, graph, feat, optimizer, max_epoch, device, sche, mute=False):
    ...
    for epoch in epoch_iter:
        ...
        with torch.no_grad():
            ...
            if inductive:
                ...
                pred = model(graph, x)
                #print(pred[val_mask == 1].shape, labels[val_mask == 1].shape)
                val_auc, val_aupr = result(pred[val_mask].cpu(), labels[val_mask].cpu())
                val_loss = criterion(pred[val_mask], labels[val_mask])
                test_auc, test_aupr = result(pred[test_mask].cpu(), labels[test_mask].cpu())
    ...
    with torch.no_grad():
        if inductive:
            ...
            estp_test_auc, estp_test_aupr = result(pred[test_mask==1].cpu(), labels[test_mask==1].cpu())
    if mute:
    ...
    return test_aupr, best_val_aupr
